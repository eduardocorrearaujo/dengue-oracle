...
if __name__ == '__main__':
    ...
    for region, TEST_YEAR in product(regioes_estados.keys(), [2026]): 

        print(f'{region} - {TEST_YEAR}')
        df = prep.load_cases_data(filename='./data/dengue_updated.csv.gz')
        ...
        X_train, y_train = prep.generate_regional_train_samples(df, enso, TEST_YEAR, columns_to_normalize=columns_to_normalize, boxcox = boxcox)
